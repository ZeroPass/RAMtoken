...
void token::create_token(name issuer, asset maximum_supply)
{
    ...
    eosio_assert(existing == statstable.end(), "RAM token already exists" );
...
void token::open(eosio::name owner, const symbol& symbol, name ram_payer)
{
...
void token::transfer_token(name from, name to, name ram_payer, asset quantity, std::string memo)
{
